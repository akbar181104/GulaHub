...
@section('content')
<div class="container">
    <h3>Rencana Panen - {{ $petani->name }} ({{ $tahunDipilih }})</h3>

    <!-- Filter Tahun -->
    <form method="GET" action="{{ route('pabrik.rencanapanen', $petani->id) }}">
    ...
    <!-- Accordion per Bulan -->
    <div class="accordion mt-3" id="accordionRencanaPanen">
        @php
            $bulanList = [
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="modalDetailLabel{{ $item->id }}">Detail Rencana Panen</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <p><strong>Jenis Tebu:</strong> {{ $item->jenis_tebu }} Ton</p>
                                    ...
                                    <div class="text-center mt-4 pb-3">
                                        <form method="POST" action="{{ route('pabrik.ajukan', $item->id) }}">
                                            @csrf
                                            <input type="hidden" name="tanggal" id="modalTanggalInput">
                                            <input type="hidden" name="petani_id" value="{{ $petani->id }}">
                                            <button type="submit" class="btn btn-primary">Ajukan ke Petani</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
